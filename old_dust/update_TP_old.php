...
//будет ли раб?? мб нельзя сейчас менять ИД? или дб другой выше в реквесте
if (isset($_SESSION['user_id'])) {
	...
	if ($group_user_id != 1 or $group_user_id != 2){
		$msg = "need more root!";
...
<form id="signup_form" action="update_TP.php" method="POST" enctype="multipart/form-data">
<fieldset>
		<label for="for_name">Имя операции:</label>
		<input type="text" name="for_name" size="20" />
	<br />
		<label for="bio">Описание операции:</label>
		<p><textarea rows="10" cols="45" name="bio" placeholder>
		</textarea></p>
	<br/>
	<!--- 
		<input type="hidden" name="MAX_FILE_SIZE" value="2000000" />
		<label for="image_tp">Загрузите фотографию:</label>
		<input type="file" name="image_tp" size="30"/>
	<br/> 
	-->
</fieldset>
